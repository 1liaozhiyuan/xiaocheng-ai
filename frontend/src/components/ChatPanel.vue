  ...
  <section class="chat">
    <div class="messages" ref="box" @scroll="onScroll">
      <template v-for="(m, i) in messages" :key="i">
        <div v-if="m.role === 'divider'" class="divider">
          <span>{{ m.text }}</span>
        </div>
        <div v-else :class="['msg', m.role]">
          <div class="bubble">{{ m.text }}<span v-if="m.typing" class="cursor">▍</span></div>
          <div class="msg-foot">
    ...
    <div class="inputbar">
      ...
      <textarea
        ...
        rows="1"
        v-model="draft"
        placeholder="和小澄说点什么…（Enter 发送，Shift+Enter 换行；说完后停顿片刻小澄会自动回应。）"
        @keydown.enter.exact.prevent="send"
        @keydown="onTyping"
        @input="onTyping(); autoGrow()"
      ></textarea>
      <button v-if="asrSupported" class="mic" :class="{ rec: recognizing }" :title="recognizing ? '正在聆听，再说一遍结束' : '语音输入'" @click="toggleMic">{{ recognizing ? '●' : '🎤' }}</button>
      <button v-if="canStop" class="stop" title="停止生成" @click="emit('stop')">■ 停止</button>
      <button v-else class="primary" @click="send" :disabled="!draft.trim() && !(listening && !streaming)">
